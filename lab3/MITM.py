...
q = int(
    "B10B8F96A080E01DDE92DE5EAE5D54EC52C99FBC"
...
def diffie_hellman_protocol():
    ...
    a_public_key = computing_public_key(g, a_private_key, q)    #YA
    ...
    # Mallory attack
    attack = Mallory()
    YA_to_b = attack.intercept_a_public_key(a_public_key)
    YB_to_a = attack.intercept_b_public_key(b_public_key)

    # shared key betwwen alice and bob but modified
    modified_a_secret = computing_shared_secret(YA_to_b, a_private_key, q)
    modified_b_secret = computing_shared_secret(YB_to_a, b_private_key, q)

    a_shared_key = derived_key(modified_a_secret)
    b_shared_key = derived_key(modified_b_secret)

    # print("shared key: ", shared_key)

    # Alice's msg to Bob - encrypted
    a_msg = "Hi Bob!"
    a_encrypt_msg = encrypt_msg(a_shared_key, a_msg)

    # Bob's msg to Alice - encrypted
    b_msg = "Hi Alice!"
    b_encrypt_msg = encrypt_msg(b_shared_key, b_msg)

    a_decrypt_msg = decrypt_msg(a_shared_key, a_encrypt_msg)
    b_decrypt_msg = decrypt_msg(b_shared_key, b_encrypt_msg)

    print("Decrypted Messages by Mallory:")
